...
#define FILE_FILL      "h5repack_fill.h5"
#define FILE_LAYOUTO   "h5repack_layouto.h5"
...
int generate_all_objects(void) {
    ...
    if (h5_using_default_driver(NULL)) {
        ...
        if (H5Pset_file_space_strategy(fcpl, H5F_FSPACE_STRATEGY_PAGE, false, (hsize_t)3) < 0)
            return -1;
        if (H5Pset_file_space_page_size(fcpl, (hsize_t)PAGE_SIZE_LARGE) < 0)
            return -1;
        assert(j < NELMTS(H5REPACK_FSPACE_FNAMES));
        if ((fid = H5Fcreate(H5REPACK_FSPACE_FNAMES[++j], H5F_ACC_TRUNC, fcpl, fapl)) < 0)
            return -1;
...
generate_layout(void) {
    ...
    if ((file_id = H5Fcreate(FILE_LAYOUTO, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
        ret_value = FAIL;
        goto done;
    }

    if ((type_id = H5Tcopy(H5T_STD_I32LE)) < 0) {
        ret_value = FAIL;
        goto done;
    }
