...
test_one_dataset_io(void)
{
    ...
    BEGIN_MULTIPART
    {
        ...
        {
            TESTING_2_API("synchronization using H5ESwait()");
            ...
            if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wbuf, es_id) < 0)
                PART_TEST_ERROR_API(single_dset_eswait);
            ...
            for (i = 0; i < 6; i++)
                for (j = 0; j < 10; j++)
                    if (wbuf[i][j] != rbuf[i][j]) {
                        ...
                        PART_ERROR_API(single_dset_eswait);
                    } /* end if */
        ...
        {
            TESTING_2_API("synchronization using H5Dclose()");
            ...
            /* Write the dataset asynchronously */
            if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wbuf, es_id) < 0)
                PART_TEST_ERROR_API(single_dset_dclose);
            ...
            for (i = 0; i < 6; i++)
                for (j = 0; j < 10; j++)
                    if (wbuf[i][j] != rbuf[i][j]) {
                        H5_FAILED_API();
                        printf("    data verification failed\n");
                        PART_ERROR_API(single_dset_dclose);
                    } /* end if */
        ...
        {
            TESTING_2_API("synchronization using H5Oflush_async()");
            ...
            /* Write the dataset asynchronously */
            if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wbuf, es_id) < 0)
                PART_TEST_ERROR_API(single_dset_dflush);
            ...
            for (i = 0; i < 6; i++)
                for (j = 0; j < 10; j++)
                    if (wbuf[i][j] != rbuf[i][j]) {
                        H5_FAILED_API();
                        printf("    data verification failed\n");
                        PART_ERROR_API(single_dset_dflush);
                    } /* end if */
        ...
        {
            TESTING_2_API("synchronization using H5Fclose()");
            ...
            /* Write the dataset asynchronously */
            if (H5Dwrite_async(dset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, wbuf, es_id) < 0)
                PART_TEST_ERROR_API(single_dset_fclose);
            ...
            for (i = 0; i < 6; i++)
                for (j = 0; j < 10; j++)
                    if (wbuf[i][j] != rbuf[i][j]) {
                        H5_FAILED_API();
                        printf("    data verification failed\n");
                        PART_ERROR_API(single_dset_fclose);
                    } /* end if */
...
test_multi_dataset_io(void)
{
    ...
    BEGIN_MULTIPART
    {
        ...
        {
            TESTING_2_API("keeping datasets open");
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((chars_written = snprintf(dset_name, ASYNC_API_TEST_FILENAME_SIZE, "dset%d", i)) < 0 ||
                    ...
                    PART_TEST_ERROR_API(multi_dset_open);
                ...
                for (j = 0; j < 6; j++)
                    for (k = 0; k < 10; k++)
                        if (wbuf[i][j][k] != rbuf[i][j][k]) {
                            H5_FAILED_API();
                            printf("    data verification failed\n");
                            PART_ERROR_API(multi_dset_open);
                        } /* end if */
        ...
        {
            TESTING_2_API("closing datasets between I/O");
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((chars_written = snprintf(dset_name, ASYNC_API_TEST_FILENAME_SIZE, "dset%d", i)) < 0 ||
                    chars_written >= ASYNC_API_TEST_FILENAME_SIZE)
                    PART_TEST_ERROR_API(multi_dset_close);
                ...
                for (j = 0; j < 6; j++)
                    for (k = 0; k < 10; k++)
                        if (wbuf[i][j][k] != rbuf[i][j][k]) {
                            H5_FAILED_API();
                            printf("    data verification failed\n");
                            PART_ERROR_API(multi_dset_close);
                        } /* end if */
...
test_multi_file_dataset_io(void)
{
    ...
    BEGIN_MULTIPART
    {
        ...
        {
            TESTING_2_API("keeping files and datasets open");
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((chars_written = snprintf(file_name, ASYNC_API_TEST_FILENAME_SIZE, ASYNC_API_TEST_FILE_PRINTF, i)) < 0 ||
                    chars_written >= ASYNC_API_TEST_FILENAME_SIZE)
                    PART_TEST_ERROR_API(multi_file_dset_open);
                ...
                for (j = 0; j < 6; j++)
                    for (k = 0; k < 10; k++)
                        if (wbuf[i][j][k] != rbuf[i][j][k]) {
                            H5_FAILED_API();
                            printf("    data verification failed\n");
                            PART_ERROR_API(multi_file_dset_open);
                        } /* end if */
        ...
        PART_BEGIN(multi_file_dset_dclose)
        {
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((dset_id[0] = H5Dopen_async(file_id[i], "dset", H5P_DEFAULT, es_id)) < 0)
                    PART_TEST_ERROR_API(multi_file_dset_dclose);
                ...
                for (j = 0; j < 6; j++)
                    for (k = 0; k < 10; k++)
                        if (wbuf[i][j][k] != rbuf[i][j][k]) {
                            H5_FAILED_API();
                            printf("    data verification failed\n");
                            PART_ERROR_API(multi_file_dset_dclose);
                        } /* end if */
        ...
        {
            TESTING_2_API("closing files between I/O");
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((chars_written = snprintf(file_name, ASYNC_API_TEST_FILENAME_SIZE, ASYNC_API_TEST_FILE_PRINTF, i)) < 0 ||
                    chars_written >= ASYNC_API_TEST_FILENAME_SIZE)
                    PART_TEST_ERROR_API(multi_file_dset_fclose);
...
test_multi_file_grp_dset_io(void)
{
    ...
    BEGIN_MULTIPART
    {
        ...
        {
            TESTING_2_API("without intermediate calls to H5ESwait()");
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((chars_written = snprintf(file_name, ASYNC_API_TEST_FILENAME_SIZE, ASYNC_API_TEST_FILE_PRINTF, i)) < 0 ||
                    (chars_written >= ASYNC_API_TEST_FILENAME_SIZE))
                    PART_TEST_ERROR_API(multi_file_grp_dset_no_kick);
                ...
                for (j = 0; j < 6; j++)
                    for (k = 0; k < 10; k++)
                        if (wbuf[i][j][k] != rbuf[i][j][k]) {
                            H5_FAILED_API();
                            printf("    data verification failed\n");
                            PART_ERROR_API(multi_file_grp_dset_no_kick);
                        } /* end if */
        ...
        {
            TESTING_2_API("with intermediate calls to H5ESwait() (0 timeout)");
            ...
            for (i = 0; i < 5; i++) {
                ...
                if ((chars_written = snprintf(file_name, ASYNC_API_TEST_FILENAME_SIZE, ASYNC_API_TEST_FILE_PRINTF, i)) < 0 ||
                    chars_written >= ASYNC_API_TEST_FILENAME_SIZE)
                    PART_TEST_ERROR_API(multi_file_grp_dset_kick);
                ...
                for (j = 0; j < 6; j++)
                    for (k = 0; k < 10; k++)
                        if (wbuf[i][j][k] != rbuf[i][j][k]) {
                            H5_FAILED_API();
                            printf("    data verification failed\n");
                            PART_ERROR_API(multi_file_grp_dset_kick);
                        } /* end if */
...
test_set_extent(void)
{
    ...
    TESTING_API("H5Dset_extent() and H5Dget_space()");
    ...
    for (i = 0; i < 6; i++) {
        if ((tri_ret = H5Sextent_equal(fspace_id[i], fspace_out[i])) < 0)
            TEST_ERROR;
        if (!tri_ret)
            FAIL_PUTS_ERROR_API("    dataspaces are not equal\n");
    ...
    for (i = 0; i < 6; i++)
        for (j = 0; j < 10; j++)
            if (wbuf[i][j] != rbuf[i][j])
                FAIL_PUTS_ERROR_API("    data verification failed\n");
...
test_attribute_exists(void)
{
    ...
    if (exists1)
        FAIL_PUTS_ERROR_API("    H5Aexists returned TRUE for an attribute that should not exist");
    if (!exists2)
        FAIL_PUTS_ERROR_API("    H5Aexists returned FALSE for an attribute that should exist");
...
test_attribute_io_tconv(void)
{
    ...
    TESTING_API("attribute I/O with type conversion");
...
test_attribute_io_compound(void)
{
    ...
    TESTING_API("attribute I/O with compound type conversion");
...
test_group(void)
{
    ...
    /* Verify group infos */
    if (info1.nlinks != 0)
        FAIL_PUTS_ERROR_API("    incorrect number of links");
    if (vol_cap_flags_g & H5VL_CAP_FLAG_CREATION_ORDER) {
        if (info2.nlinks != 1)
            FAIL_PUTS_ERROR_API("    incorrect number of links");
    }
...
test_link(void)
{
    ...
    if (!existsh1)
        FAIL_PUTS_ERROR_API("    link exists returned FALSE for link that should exist");
    if (!existss1)
        FAIL_PUTS_ERROR_API("    link exists returned FALSE for link that should exist");
    if (!existsh2)
        FAIL_PUTS_ERROR_API("    link exists returned FALSE for link that should exist");
    if (existss2)
        FAIL_PUTS_ERROR_API("    link exists returned TRUE for link that should not exist");
    if (existsh3)
        FAIL_PUTS_ERROR_API("    link exists returned TRUE for link that should not exist");
    if (existsh3)
        FAIL_PUTS_ERROR_API("    link exists returned TRUE for link that should not exist");
...
test_ocopy_orefresh(void)
{
    ...
    TESTING_API("H5Ocopy() and H5Orefresh()");
